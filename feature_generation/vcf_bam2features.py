...
def run_pysamstats_variation(bamFile,refFile,sampleName,baseq,mapq,record):
    ...
    ref = record.REF
    alt = record.ALT[0]
    for rec in pysamstats.stat_variation_strand(bam_to_process, refFile, chrom=chromosome, start=position, end=None, min_mapq=mapq, min_baseq=baseq, no_del=False, no_dup=False, one_based=True, truncate=True):
        rec['alt'] = alt
        rec['pos'] = position
...
def run_pysamstats_baseq(bamFile,refFile,baseq,mapq,sampleName,record):
    ...
    ref = record.REF
    alt = record.ALT[0]
    for rec in pysamstats.stat_baseq_ext(bam_to_process, refFile, chrom=chromosome, start=position, end=None, min_mapq=mapq, min_baseq=baseq, no_del=False, no_dup=False, one_based=True, truncate=True):
        rec['alt'] = alt
        rec['pos'] = position
        rec['Tumor_Sample_Barcode'] = sampleName
        rec = collections.OrderedDict(sorted(rec.items(),key=lambda i:keyorder.index(i[0])))
...
def run_pysamstats_gc(bamFile,refFile,baseq,mapq,sampleName,record):
    ...
    ref = record.REF
    alt = record.ALT[0]
    for rec in pysamstats.stat_coverage_gc(bam_to_process, refFile, chrom=chromosome, start=position, end=None, min_mapq=mapq, min_baseq=baseq, no_del=False, no_dup=False, window_size=50, window_offset=10, one_based=True, truncate=True):
        rec['ref'] = ref
        rec['alt'] = alt
        rec['pos'] = position
        rec['Tumor_Sample_Barcode'] = sampleName
